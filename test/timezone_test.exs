defmodule Quantum.TimezoneTest do
  ...
  test "check timezones" do
    ...
    assert execute({~e[0 21 15 * *], &ok/0, "Etc/GMT+2"}, %{date: ~N[2015-12-15 23:00:00]}) == :ok
    assert execute({~e[0 21 15 * *], &ok/0, "Etc/GMT+3"}, %{date: ~N[2015-12-16 00:00:00]}) == :ok
    assert execute({~e[0 21 15 * *], &ok/0, "Etc/GMT+3"}, %{date: ~N[2015-12-15 00:00:00]}) == false
  end

  test "check timezone with weekly" do
    assert execute({~e[@weekly], &ok/0, "Etc/GMT-3"}, %{date: ~N[2016-03-05 21:00:00]}) == :ok
    assert execute({~e[@weekly], &ok/0, "Etc/GMT-3"}, %{date: ~N[2016-03-06 00:00:00]}) == false
  end

  test "check timezone with monthly" do
    assert execute({~e[@monthly], &ok/0, "Etc/GMT-3"}, %{date: ~N[2016-02-29 21:00:00]}) == :ok
